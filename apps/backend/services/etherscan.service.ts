...
export class EtherscanHistoryService {
  async getHistoricalWhaleData(
    ...
    console.log(
      `📜 Etherscan: Querying historical data for ${walletAddress}`,
      `   Action type: ${action} | Token: ${token}`,
    );
    ...
    console.log(
      `✅ Etherscan: Retrieved ${historicalTxs.length} historical transactions`,
    );
  ...
  private generateSocialSentiment(
    ...
    return {
      ...
      reddit: {
        ...
        topSubreddits: ["r/ethereum", "r/cryptocurrency", "r/CryptoMarkets"],
      },
      forums: {
        sentiment:
...
let etherscanHistoryService: EtherscanHistoryService | null = null;

export function getEtherscanHistoryService(): EtherscanHistoryService {
  if (!etherscanHistoryService) {
    etherscanHistoryService = new EtherscanHistoryService();
  }
